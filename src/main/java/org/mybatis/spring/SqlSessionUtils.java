 ...
 * <p>
 * 该工具类个人感觉借鉴了 {@link DataSourceUtils}
 *
...
public final class SqlSessionUtils {
   ...
   * <p>
   * 从 {@link SqlSessionFactory} 创建一个全新的 {@link  SqlSession}。
   * <p>
   * 其实底层调用的方法未必会创建新的 SqlSession，看是否在事务中
   *
  ...
  public static SqlSession getSqlSession(SqlSessionFactory sessionFactory, ExecutorType executorType,
    ...
    // 事务上下文中，通过 SqlSessionFactory 寻找 SqlSessionHolder (可能是 null)
    SqlSessionHolder holder = (SqlSessionHolder) TransactionSynchronizationManager.getResource(sessionFactory);

    // 从 SqlSessionHolder 取出 SqlSession
    SqlSession session = sessionHolder(executorType, holder);

    // 如果上下文有一个 SqlSession，那么返回之
    if (session != null) {
      return session;
    }
    ...
    // 如果上下文没有 SqlSession，那么创建一个，并注册到上下文中
  ...
  private static void registerSessionHolder(SqlSessionFactory sessionFactory, ExecutorType executorType,
    ...
    if (TransactionSynchronizationManager.isSynchronizationActive()) {
      ...
      if (environment.getTransactionFactory() instanceof SpringManagedTransactionFactory) {
        ...
        // 将 holder 绑定到事务上下文中 [其实下面的钩子函数也是绑定 holder，只不过可能是后续事务传播时挂起再恢复需要用到]
        TransactionSynchronizationManager.bindResource(sessionFactory, holder);
        TransactionSynchronizationManager // 注册事务同步钩子，当挂起、恢复时，分别解绑和绑定 SqlSession
  ...
  /**
   * 获取 SqlSessionHolder 内部的 SqlSession
   *
   * @param executorType
   * @param holder
   * @return
